...
import src.my_library.data_preprocessor as data_processor
...
def test_preprocess_data():
    dp = data_processor.DataPreprocessor( mock_sentences1, mock_dictionary)
    dp.preprocess_data()
    assert np.array_equal(dp.X, np.array(mock_count_vectors))

def test_preprocess_data_and_label():
    dp = data_processor.DataPreprocessor(mock_sentences2, mock_dictionary)
